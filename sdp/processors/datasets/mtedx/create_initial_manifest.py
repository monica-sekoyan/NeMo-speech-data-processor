...
class CreateInitialManifestMTEDX(BaseParallelProcessor):
    ...
    def prepare(self):
        ...
        if not self.already_extracted:
            ...
            self.audio_path_prefix = Path(data_folder) / self.data_split / "wav"
            self.vtt_path_prefix = Path(data_folder) / self.data_split / "vtt"
        else:
            data_folder = self.raw_data_dir
            self.audio_path_prefix = (
                Path(data_folder) / f"{self.language_id}-{self.language_id}" / 'data' / self.data_split / "wav"
            )
            self.vtt_path_prefix = (
                Path(data_folder) / f"{self.language_id}-{self.language_id}" / 'data' / self.data_split / "vtt"
            )
